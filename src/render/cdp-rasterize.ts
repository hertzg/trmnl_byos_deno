...
// HTML under a generated id, hand CDP `${origin}/__internal/render/${id}`,
// and remove the shelf entry once the PNG is back.
//
// The shelf and its serving route are entirely private to this module.
// We export a Hono sub-app that the parent composes via
// `app.route("/", cdp.app)` so the public HTTP layer never has to know
// the internal route exists or that there's a shelf behind it. If we
// ever swap this CDP-via-URL implementation for `Page.setDocumentContent`
// or anything else, the sub-app simply becomes empty and the composition
// stays the same.
