...
// the `config/live/` volume is mounted (ADR-0010). Live content lives under
// config/live/ — distinct from the baked example starters. The files inside
// are served back under `GALLERY_ASSET_PREFIX` — `server/src/main.ts` wires
// the same dir + prefix as an extra asset root on the PluginManager so the
// bytes the `<img src>` points at are actually reachable. Keep the two in sync.
export const GALLERY_IMAGES_DIR = "config/live/plugins/gallery/images";
