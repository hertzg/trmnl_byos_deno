...
// Write asset files into a temp directory; returns the dir path.
async function makeAssetsDir(files: Record<string, string | Uint8Array>): Promise<string> {
...
const trivialPlugin: Plugin<unknown> = {
  run() {
    return {
      state: { ok: true },
      validity: fiveMin,
...
Deno.test("a file in assets/ becomes /assets/<name> keyed against its bytes", async () => {
  const assetsDir = await makeAssetsDir({ "foo.svg": "<svg/>" });

  const manager = await createPluginManager({ plugin: trivialPlugin, assetsDir });
  const bundle = await manager.run(ctx());
...
Deno.test("binary file bytes survive the round-trip intact", async () => {
  ...
  const assetsDir = await makeAssetsDir({ "icon.png": bytes });
...
Deno.test("a file inside a nested subdirectory keeps its full sub-path", async () => {
  const assetsDir = await makeAssetsDir({
    ...
    "icons/bell.svg": "<svg id=bell/>",
    "icons/transit/bus.svg": "<svg id=bus/>",
  });
...
Deno.test("synchronous plugin.run (non-Promise return) is awaited correctly", async () => {
  ...
  const syncPlugin: Plugin<unknown> = {
    run(_ctx) {
      return {
        state: "sync",
        validity: fiveMin,
...
Deno.test("plugin is not invoked during createPluginManager — only on run()", async () => {
  // Asset scanning is a constructor cost; the Plugin itself must not be called
  // until the first run() so module-level side effects (e.g. Transport's BVG
  // background timer) are not triggered by mere construction.
  let runCount = 0;
  const plugin: Plugin<unknown> = {
    run() {
      runCount++;
      return { state: null, validity: fiveMin, view: () => "" };
    },
  };
  await createPluginManager({ plugin, assetsDir: "/no/such/assets/dir" });
  assertEquals(runCount, 0);
});

Deno.test("each run() invokes plugin.run exactly once", async () => {
  let runCount = 0;
  const plugin: Plugin<unknown> = {
    run() {
      runCount++;
      return { state: runCount, validity: fiveMin, view: () => "" };
    },
  };
  const manager = await createPluginManager({ plugin, assetsDir: "/no/such/assets/dir" });
...
Deno.test("a PluginManager drives /api/display end-to-end through the Conductor", async () => {
  // Hermetic smoke test: construct a PluginManager + Conductor like main.ts
  // does, hit /api/display, and assert the BYOS-shaped JSON falls out the
  // other side.
  const plugin: Plugin<unknown> = {
    run(ctx) {
      return {
        ...
        view(s) {
          return "<p>" + (s as { intent: string }).intent + "</p>";
        },
      };
    },
  };
  ...
  const pluginManager = await createPluginManager({ plugin, assetsDir });
  const now = () => T0;
  ...
  const plugin: Plugin<unknown> = {
    run(ctx) {
      return {
        ...
        view(s) {
          const st = s as { intent: string; anchor: string };
          return "<p>" + st.intent + ":" + st.anchor + "</p>";
        },
      };
    },
  };
...
Deno.test("end-to-end: /api/display → /image/<id>.png drives PluginManager → Renderer.rasterize through the production wiring and CDP fetches the loopback origin", async () => {
  ...
  const plugin: Plugin<unknown> = {
    run() {
      return {
        ...
        validity: fiveMin,
        view(s) {
          return "<p>" + (s as { msg: string }).msg + "</p>";
        },
      };
    },
  };
  const assetsDir = await makeAssetsDir({ "style.css": ".x { color: red; }" });
  const pluginManager = await createPluginManager({ plugin, assetsDir });
  ...
  try {
    ...
    assertEquals(seen.html, "<!DOCTYPE html><p>ahoy</p>");
